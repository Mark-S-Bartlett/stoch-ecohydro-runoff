...
import src.data.noaa_ftp as noaa_ftp

urls = ['http://noaa-ghcn-pds.s3.amazonaws.com',
...
class Stations:
    ...
    def __init__(self, data_folder='data/', debug=False):
        ...
        for url in urls:
            try:
                # Try to fetch content from the current URL
                f = 'ghcnd-countries.txt'
                content_countries = requests.get(url + '/' + f).content.decode('utf-8')

                f = 'ghcnd-states.txt'
                content_states = requests.get(url + '/' + f).content.decode('utf-8')

                f = 'ghcnd-stations.txt'
                content_stations = requests.get(url + '/' + f).content.decode('utf-8')

                # If we reach this point without errors, break out of the loop
                break
            except requests.exceptions.RequestException as e:
                # If an error occurs, continue to the next URL
                print(f"Error fetching data from {url}: {e}")

        # TODO: add error handling for reading the file
        if self.debug:
            print('Reading ghcnd-stations.txt')
        self.df = pd.read_fwf(StringIO(content_stations), header=None, delimiter=' '
        ...
        self.countries = pd.read_fwf(StringIO(content_countries), header=None,
                            delimiter=' ', names=['CountryCode','CountryName'])

        if self.debug:
            print('Reading ghcnd-states.txt')
        self.states = pd.read_fwf(StringIO(content_states), header=None,
...
class Inventory:
    ...
    def __init__(self, Stations, data_folder='data/', debug=False):
        ...
        for url in urls:
            try:
                # Try to fetch content from the current URL
                f = 'ghcnd-inventory.txt'
                content_inventory = requests.get(url + '/' + f).content.decode('utf-8')
            except requests.exceptions.RequestException as e:
                # If an error occurs, continue to the next URL
                print(f"Error fetching data from {url}: {e}")
        ...
        self.df_inventory = pd.read_fwf(StringIO(content_inventory), header=None, delimiter=' '
